...
export const searchItems = [
  ...
  { story: 'TokenBalance', section: 'Custom Address (read-only)', description: 'No wallet needed' },
...
export const componentDescriptions: Record<string, ComponentDescription> = {
  TxKitProvider: {
    ...
    features: [
      ...
      'Theme switching via CSS classes (.tx-light / .tx-dark / .tx-sharp / .tx-soft / .tx-rounded)',
  ...
  FlowSteps: {
    summary: 'Step indicator that reads its flow entry from TxKitProvider context. Drop it anywhere - sidebars, headers, modals - via a shared flowId.',
    features: [
      'Zero prop drilling - subscribes to the shared flow store via flowId',
      'Vertical (default) and horizontal orientations',
      'Tier 2 render prop receives steps with id / label / status / isCurrent',
      'aria-live status region announces step transitions to screen readers',
    ],
    useWhen: 'You want a canonical step indicator next to TransactionButton, in a sidebar, or as a stripped-down headless render via useFlowState.',
    parent: 'TransactionButton',
    docsPath: '/api/flow-steps',
  },
  FlowProgress: {
    summary: 'Progress bar that mirrors the active flow status. Shimmers while running, switches to terminal colors on completion or error.',
    features: [
      'Slim default visual - one bar with status-driven fill',
      'Optional summary row with currentStepLabel and percent complete',
      'Shimmer animation while running, solid fill on terminal status',
      'Tier 2 render prop receives progress (0-1), status, currentStepLabel',
      'Headless via useFlowState for fully custom progress widgets',
    ],
    useWhen: 'You need a compact, layout-friendly progress signal in a header, modal toolbar, or sidebar - separate from the transaction button itself.',
    parent: 'TransactionButton',
    docsPath: '/api/flow-progress',
  },
  FlowToast: {
    summary: 'Toast notification that portals to document.body and surfaces only on terminal flow statuses (completed, error, rejected, canceled).',
    features: [
      'Portal-based render - place it anywhere, it lifts to document.body',
      'Four corner positions: top-right, top-left, bottom-right (default), bottom-left',
      'autoDismiss in milliseconds (default 5000); errors never auto-dismiss (WCAG 2.2.1)',
      'Hover or focus pauses the dismiss timer',
      'Multiple flowIds = multiple parallel toasts, each scoped to its own flow',
    ],
    useWhen: 'Mount one (or more, per-flow) at app root for end-of-flow notifications. For non-toast UI surface progress through FlowSteps or FlowProgress instead.',
    parent: 'TransactionButton',
    docsPath: '/api/flow-toast',
  },
  ThemeShowcase: {
    ...
    features: [
      'Four brand hues: indigo (canonical), violet, blue, pink',
