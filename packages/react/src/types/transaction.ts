...
import type { FlowToastLabels } from '../components/FlowToast/labels'
import type { FlowStepsLabels } from '../components/FlowSteps/labels'


/**
 * Internal wagmi mutation handles consumed by flow execution helpers.
 * The underlying wagmi generics are resolved at call sites; consolidating
 * the loose `any` shape here keeps the lint-disabled escape hatches in a
 * single place rather than spreading them across executeFlowLoop /
 * executeTxStep / executeSignStep.
 */
export type WagmiMutations = {
  /* eslint-disable @typescript-eslint/no-explicit-any -- wagmi generic signatures are not worth re-exporting */
...
export type TransactionButtonProps = {
  ...
  label?: string
  /** Optional description shown above the button (visually grouped with the action). */
  description?: string
  /** UI label overrides */
...
export type FlowStepsProps = {
  ...
  flowId?: string
  /** Override the screen-reader status copy and the "In Progress" pill */
  labels?: Partial<FlowStepsLabels>
  /** Orientation of step indicators. @default 'vertical' */
...
export type FlowProgressProps = {
  ...
  /** Flow ID to display. @default '__default__' */
  flowId?: string
  /** Show a summary row "Overall Progress - X%" above the bar. @default true */
  showSummary?: boolean
...
export type FlowToastProps = {
  ...
  /** Flow ID to display. @default '__default__' */
  flowId?: string
  /** Override toast titles, descriptions and the dismiss aria-label */
  labels?: Partial<FlowToastLabels>
