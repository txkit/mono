...
Runnable TypeScript examples demonstrating `@txkit/tx-protocol` v0.1 usage.
...
pnpm exec tsx packages/tx-protocol/examples/uniswap-permit2-swap.ts
pnpm exec tsx packages/tx-protocol/examples/safe-delegatecall-warning.ts
```
...
- [`uniswap-permit2-swap.ts`](./uniswap-permit2-swap.ts) - two envelopes: a `signature` envelope (Permit2 EIP-712) followed by an `evm-tx` envelope (Uniswap Universal Router swap). Showcases the `kind: 'signature'` flow which v0.1 introduced to stop blind-signing.
...
- [`spec/v0.1/prepared-transaction.md`](../spec/v0.1/prepared-transaction.md) - canonical specification
- [`packages/tx-protocol/README.md`](../README.md) - package API reference
